...
struct MediaItemView: View {
    ...
    var body: some View {
        ...
        .onDisappear {
            #if DEBUG
            print("🛑 MediaItemView[\(currentIndex ?? -1)] - onDisappear: 停止播放并清理")
            #endif
            ...
            // 4. 重置状态（视情况而定，如果是完全退出，重置是有益的）
            // 注意：如果在 TabView 中只是滑出去，重置 hasAutoPlayedForSelection 意味着滑回来会再次自动播放
            // 如果希望滑回来不自动播放，不要重置 hasAutoPlayedForSelection
            // 这里我们只重置播放状态，不重置“已自动播放过”的标记，以免滑回来又自动播
            mediaItemWrapper.currentPlayToken = nil
            mediaItemWrapper.handledPlayToken = nil
        }
    }
